...
class Cryptid(ShaderInterface):
    name = "Cryptid"

    def params(self):
        return {'resolutionX_Cryptid' : 1, 'resolutionY_Cryptid' : 1, 'strength_Cryptid' : 1}
    ...
    def fragShader(self):
        return  '''
                    ...
                    uniform float resolutionX_Cryptid;
                    uniform float resolutionY_Cryptid;
                    uniform float strength_Cryptid;
                const vec2 resolutionX_Cryptid_Range = vec2(screenWidth/4, screenWidth);
                const vec2 resolutionY_Cryptid_Range = vec2(screenHeight/4, screenHeight); 
                const vec2 strength_Cryptid_Range = vec2(0, 1);
                vec2 resolution_Cryptid = vec2(scale(resolutionX_Cryptid, resolutionX_Cryptid_Range), scale(resolutionY_Cryptid, resolutionY_Cryptid_Range));

                float strength_Cryptid_Scaled = scale(strength_Cryptid, strength_Cryptid_Range);


                    vec4 frag_color_Cryptid;
                    vec4 color_Cryptid;

                    // shadertoy emulation
                    #define iresolution_Cryptid resolution_Cryptid
                    ...
                    void mainImage_Cryptid( out vec4 fragColor, in vec2 fragCoord )
                    {
                      vec2 uv = vec2(fragCoord.x / iresolution_Cryptid.x, fragCoord.y / iresolution_Cryptid.y);
                      uv -= 0.5;
                      uv /= vec2(iresolution_Cryptid.y / iresolution_Cryptid.x, 1);
                      ...
                      fragColor = vec4(col, 1) * strength_Cryptid_Scaled;
                      fragColor += frag_color_Cryptid;

                    }
                    // --------[ Original ShaderToy ends here ]---------- //

                    void main_Cryptid(void)
                    {
                        mainImage_Cryptid(color_Cryptid, gl_FragCoord.xy);
                    }
                '''
